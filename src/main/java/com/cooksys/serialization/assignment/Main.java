...
public class Main {
	...
	public static Session readSession(File rootDirectory, JAXBContext jaxb) throws JAXBException {
		...
		File date = new File("/input/memphis/08-08-2016");
//		File inst = new File("/input/memphis/08-08-2016/instructor.xml");
//		File st = new File("/input/memphis/08-08-2016/students");
		
		se.setLocation(rootDirectory.getName());
		se.setStartDate(date.getName());
		se.setInstructor(readInstructor(new File("input/memphis/08-08-2016/instructor.xml"), context));
		se.setStudents(readStudents(new File("input/memphis/08-08-2016/students"), context));
	 ...
	 * com.cooksys.serialization.assignment.model package 
	 * 2. Read a session
